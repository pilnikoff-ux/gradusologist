...
export const MyJournalModal: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div
        className="relative w-full max-w-4xl max-h-[92vh] bg-zinc-900 border border-zinc-700/80 rounded-3xl shadow-2xl flex flex-col overflow-hidden text-zinc-100 my-auto min-w-0"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="flex items-center justify-between px-3.5 sm:px-8 py-3 sm:py-5 border-b border-zinc-800 bg-zinc-950/80 min-w-0 gap-2">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-sm sm:text-xl font-black font-['Unbounded'] text-white flex items-center gap-1.5 sm:gap-2 flex-wrap">
                <span className="truncate">{isUa ? 'Особистий Журнал' : 'Personal Journal'}</span>
                <span className="text-[10px] bg-amber-500/20 text-amber-400 border border-amber-500/40 px-2 py-0.5 rounded-full font-mono shrink-0">
                  {favorites.length + journal.length} {isUa ? 'збережено' : 'saved'}
                </span>
              </h2>
              <p className="text-[11px] sm:text-xs text-zinc-400 truncate">
                {isUa
                  ? 'Збережені улюблені коктейлі та аномальні рецепти'
                  : 'Saved favorite cocktails & impossible recipes'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            {currentUser ? (
              <button
                ...
                className="px-2 sm:px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-emerald-500/40 text-xs text-stone-200 flex items-center gap-1.5 sm:gap-2 transition-all cursor-pointer"
              ...
              <button
                ...
                className="px-2 sm:px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 hover:border-amber-500/50 text-xs text-stone-300 hover:text-amber-400 flex items-center gap-1 sm:gap-1.5 transition-all cursor-pointer"
              >
                {/* Small Google icon */}
            ...
            <button
              ...
              className="p-1.5 sm:p-2 rounded-xl bg-zinc-800/80 hover:bg-zinc-700 text-zinc-400 hover:text-white transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* Tab Navigation */}
        <div className="flex flex-wrap items-center justify-between px-3.5 sm:px-8 py-2.5 sm:py-3 bg-zinc-900 border-b border-zinc-800 gap-2 min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
            <button
              ...
              className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-sm font-bold font-['Unbounded'] transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer ${
                activeTab === 'favorites'
                  ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20'
                  : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'
              }`}
            >
              <Heart className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'favorites' ? 'fill-black' : 'text-rose-400'}`} />
            ...
            >
              <BookOpen className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-rose-300" />
              <span>{isUa ? '«Це пиздець»' : 'Madness Recipes'}</span>
          ...
          {activeTab === 'favorites' && favorites.length > 0 && onClearFavorites && (
            <button
              ...
              className="text-xs text-zinc-400 hover:text-rose-400 flex items-center gap-1.5 transition-colors cursor-pointer ml-auto"
            >
