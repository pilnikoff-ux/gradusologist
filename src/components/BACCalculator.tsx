...
export const BACCalculator: React.FC<Props> = ({ language }) => {
  ...
  return (
    <section id="bac-calculator-section" className="py-8 sm:py-12 px-3 sm:px-6 max-w-6xl mx-auto w-full max-w-full overflow-hidden">
      {/* Header */}
      <div className="text-center max-w-3xl mx-auto mb-6 sm:mb-8 min-w-0">
        ...
        <p className="mt-2 text-stone-400 text-xs sm:text-base">
          {isUa
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start w-full min-w-0">
        ...
        <div className="lg:col-span-7 rounded-3xl bg-stone-900 border border-stone-800 p-4 sm:p-8 space-y-5 sm:space-y-6 shadow-2xl w-full min-w-0">
          {/* User Parameters */}
          ...
          <div className="min-w-0">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
              <button
                ...
                className={`py-2 px-2 sm:px-3 rounded-xl text-[11px] sm:text-xs font-semibold border transition-all text-center truncate ${
                  stomachState === 'empty'
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
              {presetDrinkOptions.map((p, idx) => (
                <button
                  ...
                  className="p-2 sm:p-2.5 rounded-xl bg-stone-950 hover:bg-stone-800 border border-stone-800 hover:border-cyan-500/50 text-left text-xs text-stone-300 transition-colors flex items-center justify-between min-w-0"
                >
                  <span className="truncate min-w-0">{isUa ? p.name : p.nameEn}</span>
            ...
            {drinks.length === 0 ? (
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                {drinks.map((d) => (
                  <div
                    key={d.id}
                    className="px-3 py-2 rounded-xl bg-stone-950 border border-stone-800 flex items-center justify-between text-xs min-w-0 gap-2"
                  >
                    <span className="text-stone-200 truncate min-w-0">{isUa ? d.name : d.nameEn}</span>
                    <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                      <span className="font-mono text-cyan-400 text-[11px] sm:text-xs">
                        {d.volumeMl} {isUa ? 'мл' : 'ml'} ({d.abv}%)
                      </span>
                      <button
                        onClick={() => handleRemoveDrink(d.id)}
                        className="text-stone-500 hover:text-rose-400 cursor-pointer p-0.5"
                      >
        ...
        <div className="lg:col-span-5 rounded-3xl bg-gradient-to-b from-stone-900 via-stone-900 to-stone-950 border-2 border-cyan-500/40 p-4 sm:p-8 space-y-5 sm:space-y-6 shadow-2xl text-center relative overflow-hidden w-full min-w-0">
          ...
          <div className="py-2 sm:py-4">
            <div className={`text-5xl sm:text-7xl font-black font-['Unbounded'] ${dangerColor} transition-colors`}>
          ...
          <div className="p-3.5 sm:p-4 rounded-2xl bg-stone-950 border border-stone-800 text-left space-y-3 min-w-0">
            <div className="flex items-start gap-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-stone-300 mt-0.5 leading-relaxed break-words">
                  {bacPromille <= 0.2
                    ? isUa
